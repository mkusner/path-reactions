...
\begin{columns}
 ...
 % }
  ...
  \block{Reactions can be represented by movements of electrons: the reaction mechanism}{
    \begin{cols}
    \col{.54}
    ...
    \col{.44}
      \begin{itemize}
        \item Reactions involve the stepwise movement of electrons along the atoms: this \colorize{removes} and \colorize{adds} bonds.
        \item Recent approaches of ML applied to reaction prediction \citep{jin2017predicting,schwaller2017found} have focused on product prediction, we instead approach the problem from the angle
          of mechanism prediction: this provides \colorize{interprebility}, takes advantage of the \colorize{sparsity} of reactions, and naturally \colorize{incorporates the constraints of chemistry} (eg balanced atom counts).
        \item We model reactions exhibiting linear electron flow (LEF) topology, the most common occuring of reactions \citep{herges1994coarctate}.
      \end{itemize}
    \end{cols}
 ...
 Reactions exhibiting LEF topology have a single electron path consisting of alternating \colorize{add} and \colorize{remove} steps.
  
    \begin{algorithm}[H]
    %\caption{}
    \begin{algorithmic}[1]
      \State Select \colorize{starting} atom A using 
      ...
      \MRepeat
        ...
        and \colorize{remove} 2 electrons from bond B-A.
        \State Decide whether to \colorize{continue} using $p_\theta^{\textrm{cont}}(c_{t+1} \mid \moleculeSet_{t+1})$.
        \State Select new atom A using $p_\theta^{\mathrm{add}}(a_t | \moleculeSet_t, a_{t-1})$ and \colorize{add} bond B-A.
        \State Decide whether to \colorize{continue} using $p_\theta^{\textrm{cont}}(c_{t+1} \mid \moleculeSet_{t+1})$.
      \EndRepeat
    \end{algorithmic}
    \end{algorithm}
...
\begin{columns}
  ...
  \block{Assessing \textsc{ELECTRO} for product prediction}{
    ...
    Assessing only for predicting mechanisms means that predicting the correct product in an alternative way is labelled as incorrect.
    ...
    \col{0.06}
    \col{0.6}
    \begin{center}  $\therefore$ We also evaluate it for product prediction.
    \end{center}
...
%% References
  \block{}{
    ...
    Philippe Schwaller  and Théophile Gaudin  and Dávid Lányi  and Costas Bekas  and Teodoro Laino.
    ...
    Matthew A. Kayala and Pierre F. Baldi.
    ...
    D. M. Lowe.
    \newblock Patent reaction extraction: downloads.
    \newblock https://bitbucket.org/dan2097/patent-reaction-extraction/downloads, 2014.
