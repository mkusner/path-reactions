...
\replaced[id=jab]{%
These works require expert-defined features and have so far been run on carefully, expert-curated datasets, for which organic chemists have to hand-code every electron pushing step.
 In contrast in our work, we aim to learn reactions from noisy large-scale reaction datasets.
 As  we cannot yet use the above methods on these datasets, we cannot compare directly against them, and we leave a detailed discussion of the pros and cons of each method for future work.
}{
...
%
Reaction mechanisms can be classified by the topology of their ``electron-pushing arrows'' (the red and green arrows in Figure~\ref{fig:task-overview}). Here, the class of reactions with \emph{linear electron flow} (LEF) topology is by far the most \replaced[id=jab]{common and fundamental}{important}, followed by those with cyclic topology \citep{herges1994coarctate}. In this work, we will only consider LEF reactions that are \emph{heterolytic}, i.e., they involve pairs of electrons, \added[id=jab]{as opposed to homolytic reactions which involve single electrons.}
...
\added[id=jab,remark={for reviewer 3's nit.}]{In arrow pushing diagrams representing LEF reactions, this electron path can be represented by arrows that line up in sequence, differing from for example cyclic reactions in which the arrows would form a loop \citep{herges1994coarctate}.}
