...
As we cannot yet use the above methods on the datasets we use, nor test our models on the datasets they use, as they are not yet publicly released, we cannot compare directly against them, and we leave a detailed discussion of the pros and cons of each method for future work.
}{
...
Here, the class of reactions with \emph{linear electron flow} (LEF) topology is by far the most \replaced[id=jab]{common and fundamental}{important}, followed by those with cyclic topology \citep{herges1994coarctate}. 
In this work, we will only consider LEF reactions that \replaced[id=ms]{}{are \emph{heterolytic}, i.e., they }involve pairs of electrons.\footnote{The treatment of radical reactions, which involve the movement of single electrons, will be the topic of future work.}
...
\added[id=jab,remark={for reviewer 3's nit.}]{In arrow pushing diagrams representing LEF reactions, this electron path can be represented by arrows that line up in sequence, differing from for example pericyclic reactions in which the arrows would form a loop \citep{herges1994coarctate}.}

Further \replaced[id=jab,remark={for reviewer 3's nit.}]{for LEF reactions, the movement of the electrons along the linear path}{, the electron path} will alternately remove existing bonds and form new ones. 
