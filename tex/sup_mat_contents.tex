...
Having formed node embeddings we can use these to form graph embeddings \citep{li2018learning,Johnson2017-pd}, which, as described in the main text, are $q$-dimensional vectors representing a set of nodes; i.e.\ an entire molecule or set of molecules.
 We define the function that maps a set of node features to a graph embedding by $\fEmbedGraphs$.
...
As mentioned above (Eq. \ref{eq:graph-embedding}) both $\fEmbedGraphs_{\textrm{stop}}$ and $\fEmbedGraphs_\mathrm{reagent}$ consist of three
linear functions. 
...
Finally, $\fuk$ differs between the two modules, as for $\fEmbedGraphs_{\textrm{stop}}$ it projects down to one dimension (ie $q=1$) (to later go through a sigmoid function and compute a stop probability), whereas for  $\fEmbedGraphs_\mathrm{reagent}$, $\fuk$ projects  to a dimensionality of 100 (ie $q=100$) to form the reagent embedding.
