...
Similar to the starting location distribution we again make use of a graph-isomorphic embedding function $h_{\Ac}(\moleculeSet)$. Differently, the above distribution can be split into two distributions depending on the parity of $t$:
...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
  	...
  	%\STATE $\actionLogits = $
  	\STATE $p_\theta^{\mathrm{start}}(a \mid \moleculeSet_0, \moleculeSet_e) \triangleq \mbox{softmax}\Big[f^{\mathrm{start}}\big(h_{\Ac}(\moleculeSet_0), g^\mathrm{reagent}(\Mc_e)\big)\Big]$ \COMMENT{prob $a$ starts reaction}
  	...
  	\FOR{$t = 1, \ldots, T^\mathrm{max}$}
  		...
  		\STATE $c_t \sim p_\theta^{\textrm{cont}}(c_t \mid \moleculeSet_t)$ \COMMENT{whether to continue reaction}
  		\IF{$c_t = 0$}
  			\STATE break
  		\ENDIF
  		%\STATE \mathbf{h}_
