...
%%%%%%%%%%%%%%% 
%%%%%%%%%%%%%%% 
 s reactions involve sets of molecules, we represent 
...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
  	...
  	%\STATE $\actionLogits = $
  	\STATE $p_\theta^{\mathrm{start}}(a_t \mid \moleculeSet_0, \moleculeSet_r) \triangleq \mbox{softmax}(\fInitial(\nodeEmbeddings{\moleculeSet_0}, \contextVect_\mathrm{reagent}))$ \COMMENT{probability $a_t$ starts the reaction}
  	\STATE $a_{0} \sim p_\theta^{\mathrm{start}}(a_t \mid \moleculeSet_0, \moleculeSet_r)$
  	\FOR{$t = 1, \ldots, T^\mathrm{max}$}
  		\STATE $\moleculeSet_t \leftarrow \moleculeSet_{t-1}, a_{t-1}$ \COMMENT{modify molecules based on previous molecule and action}
  		\STATE $\nodeEmbeddings{\moleculeSet_t} = \fEmbed(\Mc_t)$ \COMMENT{atom embedding of new molecules}
  		\IF{$t$ is odd}
  			\STATE $p_\theta^{\mathrm{remove}} \triangleq \mbox{softmax}(\fRemove(\nodeEmbeddings{\moleculeSet_t}, [\nodeEmbeddings{\moleculeSet_t}]_{a_{t-1}}))$ 
  			\STATE $a_t \sim p_\theta^{\mathrm{remove}}(a_t | \moleculeSet_t, a_{t-1})$ \COMMENT{electrons remove bond between $a_t$ and $a_{t-1}$}
  		\ELSE
  		  	\STATE $p_\theta^{\mathrm{add}} \triangleq \mbox{softmax}(\fAdd(\nodeEmbeddings{\moleculeSet_t}, [\nodeEmbeddings{\moleculeSet_t}]_{a_{t-1}}))$
  		...
  		\STATE $p_\theta(c_t \mid \moleculeSet_t) \triangleq \sigma(r_{\textrm{cont}}(\nodeEmbeddings{\moleculeSet_t}))$
  		\STATE $c_t \sim p_\theta(c_t \mid \moleculeSet_t)$ \COMMENT{whether to continue reaction}
  		\IF{$c_t = 0$}
  			\STATE break
  		\ENDIF
  		%\STATE \mathbf{h}_
