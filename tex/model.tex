...
the conditional probability $p(a_t \mid a_{t-1}, \moleculeSet_t, t)$ 
...
%	\graphEmbeddings = \fuk\big(\sum_{v \in \Ac'} \left[ \mbox{sigmoid}(\fui(\Hb_{\Ac,v})) \cdot \fuj(\Hb_{\Ac,v}) \right]\big).
	\graphEmbeddings = \fuk\big(\sum_{v \in \Ac'} \sigma\left(\fui(\Hb_{\Ac,v})\right) \fuj(\Hb_{\Ac,v})\big).
...
\todo{are these "actions"? where do we first use "action" to refer to atom selection} 
...
Simply simulating from the conditional distributions until sampling a stop value $s_t$ samples electron paths.
