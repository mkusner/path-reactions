...
g^{\mathrm{reagent}}\left(h_{\Ac}(\moleculeSet_e)\right)\big)\Big], 
\end{align}
...
\begin{align}
%$
p_\theta^\textrm{cont}(c_t \mid \moleculeSet_t) = \sigma(g^{\textrm{cont}}(h_{\Ac}(\moleculeSet_t))).
%$
\end{align}
...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
  	...
  	%\STATE $\actionLogits = $
  	\STATE $p_\theta^{\mathrm{start}}(a \mid \moleculeSet_0, \moleculeSet_e) \triangleq \mbox{softmax}\Big[f^{\mathrm{start}}\big(h_{\Ac}(\moleculeSet_0), g^\mathrm{reagent}(h_{\Ac}(\Mc_e))\big)\Big]$ \COMMENT{$a$ starts reaction}
  	...
  	\FOR{$t = 1, \ldots, T^\mathrm{max}$}
  		...
  		\STATE $p_\theta^{\textrm{cont}}(c_{t+1} \mid \moleculeSet_{t+1}) \triangleq \sigma(g^{\textrm{cont}}( h_{\Ac}(\moleculeSet_{t+1})))$
