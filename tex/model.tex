...
Input molecules into the model have been Kekulized \todo[]{1. is this a proper noun and 2. I hope this has been defined by now!} so that each bond $b \in \Bc$ can be a single bond, double bond, or triple bond.
...
Finally each of the modules compute the probability vector over actions. So again for the initial step we have 
...
The $\bm{\beta}$ term is a binary vector, that allows us to mask out specific actions. This value of this differs for the {\em initial}, {\em add} and {\em remove} steps. 
For initial step any action (or atom) can be picked and so this is 1 everywhere. For the remove step $\bm{\beta_\textrm{remove}}$ masks out bonds that do not currently exist (although self bonds are allowed in the first step).
